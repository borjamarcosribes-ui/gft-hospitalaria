...
def main(argv=None):
 ...
 with SessionLocal() as db:
  ...
  by=Counter(); ex=[]; written=processed=skipped_existing=0
  for cn in cns:
   norm=normalize_cn_value(cn)
   if not norm: by['skipped_invalid_cn']+=1; continue
   existing=db.get(CimaMedicamentoCache,norm)
   if a.only_missing and existing and existing.sync_status=='ok' and not a.force and not (a.retry_errors and existing.sync_status=='error'):
    by['skipped_existing']+=1; skipped_existing+=1; continue
   processed+=1
   if a.dry_run: by['would_sync']+=1; continue
   row=sync_cn(db,norm,force=(a.force or a.retry_errors)); by[row.sync_status if row.sync_status in {'ok','not_found'} else 'error']+=1; written+=1
