...
def main(argv=None):
 ...
 if not x.skip_bifimed: out['bifimed']=_cap(b.main,[mode,*base,*(['--force'] if x.force_bifimed else []),*(['--refresh-ok'] if x.refresh_bifimed_ok else []),*(['--retry-not-found'] if x.retry_bifimed_not_found else [])])
 if not x.skip_cima_medicamento: out['cima_medicamento']=_cap(c.main,[mode,*base,*(['--force'] if x.force_cima else []),*(['--seed-from-imported-urls'] if x.seed_from_imported_urls else []),*(['--repair-not-found-from-imported-url'] if x.repair_not_found_from_imported_url else [])])
 out['clinical']=_cap(cl.main,[mode,*base,'--sections',*x.sections,'--candidate-mode','syncable',*(['--force-summary'] if x.force_summary else []),*(['--skip-sections'] if x.skip_sections else []),*(['--skip-summaries'] if x.skip_summaries else [])])
