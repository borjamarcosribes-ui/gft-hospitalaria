...
def main(argv=None):
 ...
 base=['--scope',x.scope,'--batch-size',str(x.batch_size),'--max-batches',str(x.max_batches),'--json']+sum([['--cn',cn] for cn in x.cn],[])
 ...
 before=_cap(a.main,['--scope',x.scope,'--json','--sections',*x.sections]+sum([['--cn',cn] for cn in x.cn],[]))
 ...
 out['after']=_cap(a.main,['--scope',x.scope,'--json','--sections',*x.sections]+sum([['--cn',cn] for cn in x.cn],[]))
