...
def parse_args(argv=None):
    ...
    p.add_argument('--candidate-mode', default='first', choices=['first','syncable']); p.add_argument('--retry-section-unavailable', action='store_true')
...
def main(argv=None):
    ...
    if a.dry_run:
        sync_plan={} if a.skip_sections else _parse_json_output(sync_mod.main,['--dry-run','--sections',*a.sections,'--only-missing','--candidate-mode',a.candidate_mode,'--limit',str(a.batch_size*a.max_batches),'--json','--scope',a.scope,*( ['--retry-section-unavailable'] if a.retry_section_unavailable else [])]+sum([['--cn',cn] for cn in a.cn],[]))
        ...
        for i in range(a.max_batches):
            limit=a.batch_size
            sync_args=['--confirm-write','--sections',*a.sections,'--only-missing','--candidate-mode',a.candidate_mode,'--limit',str(limit),'--json','--scope',a.scope,*( ['--retry-section-unavailable'] if a.retry_section_unavailable else [])]
            sum_args=['--confirm-write','--only-missing','--candidate-mode','summary_ready','--limit',str(limit),'--json','--scope',a.scope]
            if a.cn: sync_args=['--confirm-write','--sections',*a.sections,'--only-missing','--candidate-mode',a.candidate_mode,'--json',*( ['--retry-section-unavailable'] if a.retry_section_unavailable else [])]+sum([['--cn',cn] for cn in a.cn],[]); sum_args=['--confirm-write','--only-missing','--candidate-mode','summary_ready','--json']+sum([['--cn',cn] for cn in a.cn],[])
