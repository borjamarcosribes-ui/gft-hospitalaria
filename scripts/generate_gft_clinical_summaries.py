...
def _summary_ready_cns(db, cns: list[str], prioritize_missing_summary: bool = False) -> list[str]:
    ...
    for cn in cns:
        ...
        found = db.query(CimaFichaTecnicaCache.cn).filter(
            build_auditable_section_scope_filters([cn], list(TARGET_SECTIONS)),
        ).first()
        if found:
...
def main(argv=None) -> int:
    ...
    with SessionLocal() as db:
        ...
        for cn in base_cns:
            ...
            rows = db.query(CimaFichaTecnicaCache).filter(build_auditable_section_scope_filters([cn], list(TARGET_SECTIONS))).all()
