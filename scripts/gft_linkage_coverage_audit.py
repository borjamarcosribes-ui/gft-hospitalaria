...
def main(argv=None):
    ...
    out={
      ...
      'summaries':{'con_resumen':len(sums),'source_status_counts':dict(sstat),'con_ajuste_renal':sum(1 for r in sums.values() if (r.resumen_ajuste_renal or '').strip()),'con_ajuste_hepatico':sum(1 for r in sums.values() if (r.resumen_ajuste_hepatico or '').strip()),'con_embarazo':sum(1 for r in sums.values() if (r.resumen_embarazo or '').strip()),'con_lactancia':sum(1 for r in sums.values() if (r.resumen_lactancia or '').strip()),'pendientes_resumen':sum(1 for cn in cns if cn not in sums),'examples_pendientes_resumen':ex_pend},
