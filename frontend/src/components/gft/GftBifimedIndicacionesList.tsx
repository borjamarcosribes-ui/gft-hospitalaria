...
function financiacionBadge(financiada: boolean | null | undefined): { label: string; className: string } {
  if (financiada === true) {
    return { label: 'Financiada', className: 'gft-bifimed-indicaciones__badge gft-bifimed-indicaciones__badge--yes' };
  }

  if (financiada === false) {
    return { label: 'No financiada', className: 'gft-bifimed-indicaciones__badge gft-bifimed-indicaciones__badge--no' };
  }

  return { label: 'No informado', className: 'gft-bifimed-indicaciones__badge gft-bifimed-indicaciones__badge--unknown' };
}

export function GftBifimedIndicacionesList({
  indicaciones,
}: {
  indicaciones: GFTIndicacionAutorizadaBifimed[] | null | undefined;
}) {
      ...
      {indicaciones.map((item, idx) => {
        ...
        return (
          <details key={idx} className="gft-bifimed-indicaciones__item" role="listitem">
            <summary>
              <span>Indicación {idx + 1}</span>
              <span className={badge.className}>{badge.label}</span>
            </summary>
            <div className="gft-bifimed-indicaciones__content">
              <p>
                <strong>Indicación autorizada:</strong> {formatValue(item.indicacion_autorizada)}
              </p>
              <p>
                <strong>Situación:</strong> {formatValue(item.situacion_expediente_indicacion)}
              </p>
              <p>
                <strong>Resolución:</strong> {formatValue(item.resolucion_expediente_financiacion_indicacion)}
              </p>
            </div>
          </details>
        );
