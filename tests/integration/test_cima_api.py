...
def test_cima_sync_import_batch_endpoint(client, db_session, monkeypatch):
    batch = ImportBatch(filename='x.xlsx', status='validated')
    db_session.add(batch)
    db_session.flush()
    db_session.add(ImportRowStaging(id=uuid.uuid4(), batch_id=batch.id, row_number=1, cn_normalized='123456', estado_gft='incluido', estado_editorial='publicado', validation_errors=[]))
    db_session.commit()

    monkeypatch.setattr('app.services.cima_sync_service.CimaClient.get_by_cn', lambda self, cn: FakeOk())
    r = client.post(f'/cima/sync/import-batch/{batch.id}?force=true')
    assert r.status_code == 200
    body = r.json()
...
def test_cima_sync_import_batch_404(client):
    r = client.post(f'/cima/sync/import-batch/{uuid.uuid4()}')
