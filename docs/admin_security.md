...
Los endpoints admin e internos sensibles requieren definir la variable de entorno `ADMIN_API_KEY`.

No existe un valor por defecto para esta clave. Si `ADMIN_API_KEY` no está configurada, los endpoints protegidos responden siempre con HTTP 503 y el detalle `Admin API key is not configured`.

## Comportamiento

Para acceder a un endpoint protegido, la petición debe enviar:
...
Actualmente usan este guard los endpoints admin y los routers internos de importación/sincronización:
...
Esta protección sirve como guard temporal para endpoints internos/admin antes de exponer rutas de escritura, importación o sincronización. No debe usarse como seguridad definitiva en producción pública.
